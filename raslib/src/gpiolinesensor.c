...
typedef struct GPIOLineSensor {
  ...
  void (*BackgroundRead)(struct GPIOLineSensor *ls, tCallback callback,
                         void *data);
...
static void GPIOLineSensorBackgroundRead(tGPIOLineSensor *ls,
                                         tCallback callback, void *data) {
  int i;
...
// the GPIOLineSensor to complete, the GPIOLineSensor will read as fast as
