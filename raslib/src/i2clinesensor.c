...
typedef struct I2CLineSensor {
  ...
  void (*BackgroundRead)(struct I2CLineSensor *ls, tCallback callback,
                         void *data);
...
static void I2CLineSensorHandler(tI2CLineSensor *ls) {
  if (ls->index >= 8) {
    ...
    I2CBackgroundRequest(ls->i2c, ls->address, &ls->command, 1,
                         &ls->values[index], 1, I2CLineSensorHandler, ls);
  }
}
...
static void I2CLineSensorBackgroundRead(tI2CLineSensor *ls, tCallback callback,
                                        void *data) {
...
// the LineSensor to complete, the LineSensor will read as fast as possible
// without overlap
