 ...
 * @return Pointer to an initialized tGPIOLineSensor, can be used by the
 * GPIOLineSensorRead functions
 */
tGPIOLineSensor *_InitializeGPIOLineSensor(tPin p0, tPin p1, tPin p2, tPin p3,
                                           tPin p4, tPin p5, tPin p6, tPin p7);
 ...
 * @return Pointer to an initialized tLineSensor, can be used by the
 * LineSensorRead functions
 */
 ...
 * GPIOLineSensorRead functions
 */
tLineSensor *InitializeGPIOLineSensor(tPin p0, tPin p1, tPin p2, tPin p3,
                                      tPin p4, tPin p5, tPin p6, tPin p7) {
  return (tLineSensor *)_InitializeGPIOLineSensor(p0, p1, p2, p3, p4, p5, p6,
                                                  p7);
}

/**
 * Returns the line sensor value measured as a bit-packed byte
 * @param ls Pointer to an initialized tLineSensor, returned by
 * InitializeLineSensor
 * @param threshold If the value read from a single IR on the linesensor is
 * above this threshold, it is converted to 1 and then packed into the byte
 * @return Value measured from the line sensors as a bit-packed byte, where each
 * bit of the byte corresponds to the value of a single IR senor on the line
 * sensor array Note: if the line sensor is not continously reading, then the
 * function will busy wait for the results Note: if there is an error in the I2C
 * module, the returned value will be all ones
 */
 ...
 * @param ls Pointer to an initialized tLineSensor, returned by
 * InitializeLineSensor
 * @param array Array of 8 percentages, each corresponding to an IR sensor in
 * the line sensor array
 * @return true if successful, otherwise it returns false and fills the array
 * with infinities Note: if the line sensor is not continously reading, then the
 * function will busy wait for the results
 */
 ...
 * @param snr Pointer to an initialized tLineSensor, returned by
 * InitializeLineSensor
 * @param callback Function called the next time the line sensor read completes,
 * in which a call to LineSensorRead will return with the newly obtained value
 * immediately
 ...
 * Note: If the passed time between calls is less than the time it takes for the
 * line sensor read to complete, the line sensor will fire as fast as possible
 * without overlap
 */
