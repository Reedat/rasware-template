...
struct I2C {
  ...
  // State machine
  volatile enum { DONE, SENDING, RECEIVING, TIMEOUT } state;
...
static void InitializeI2CModule(tI2C *i2c) {
  ...
  I2CMasterIntEnableEx(i2c->BASE, I2C_MASTER_INT_TIMEOUT | I2C_MASTER_INT_DATA);
  IntEnable(i2c->INT);
}
...
#define I2C_HANDLER(MOD)                                                   \
  void i2c##MOD##_handler(void) {                                          \
    ...
    unsigned long status =                                                 \
        I2CMasterIntStatusEx(I2C##MOD##_MASTER_BASE, false);               \
                                                                           \
    if (status == I2C_MASTER_INT_TIMEOUT) {                                \
      I2CMasterIntClearEx(I2C##MOD##_MASTER_BASE, I2C_MASTER_INT_TIMEOUT); \
      i2c->state = TIMEOUT;                                                \
    } else {                                                               \
      I2CMasterIntClearEx(I2C##MOD##_MASTER_BASE, I2C_MASTER_INT_DATA);    \
    }                                                                      \
                                                                           \
    switch (i2c->state) {                                                  \
      case SENDING:                                                        \
...
void I2CBackgroundSend(tI2C *i2c, unsigned char addr, const unsigned char *data,
                       unsigned int len, tCallback callback, void *cbdata) {
...
void I2CBackgroundReceive(tI2C *i2c, unsigned char addr, unsigned char *data,
                          unsigned int len, tCallback callback, void *cbdata) {
...
static void I2CRequestHandler(tI2C *i2c) {
  ...
  I2CBackgroundReceive(i2c, i2c->request.addr, i2c->request.data,
                       i2c->request.len, i2c->request.callback,
                       i2c->request.cbdata);
}
