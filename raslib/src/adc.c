...
static void SetupContinuous(tADCModule *mod, unsigned long trigger) {
  ...
  ADCSequenceStepConfigure(mod->BASE, 0, i,
                           ADC_CTL_IE | ADC_CTL_END | CHANNELS[adc->pin]);
...
static void TriggerSingle(tADCModule *mod) {
  ...
  ADCSequenceStepConfigure(
      mod->BASE, 1, 0,
      ADC_CTL_IE | ADC_CTL_END | CHANNELS[mod->singleQueue->pin]);
...
#define ADC_S0_HANDLER(MOD)                              \
  void adc##MOD##_seq0_handler(void) {                   \
    ...
    ADCSequenceDataGet(ADC##MOD##_BASE, 0, data);        \
                                                         \
    for (; adc != 0; adc = adc->next) adc->value = *d++; \
  }
...
#define ADC_S1_HANDLER(MOD)                              \
  void adc##MOD##_seq1_handler(void) {                   \
...
void ADCTriggerHandler(tADCModule *mod) { ADCProcessorTrigger(mod->BASE, 0); }
